...
namespace Utility
{
    ...
    glm::mat4 eigenToGlm(Eigen::Transform<float, 3, Eigen::Affine> const& t);
    glm::mat4 eigenToGlm(Eigen::Transform<float, 3, Eigen::Projective> const& t);
    glm::mat4 eigenToGlm(Eigen::Matrix4f const& t);
    Eigen::Matrix4f glmToEigen(glm::mat4 const& m);
    glm::mat4 glmFromEigen(Eigen::Matrix4f const& t);
    Eigen::Matrix4f eigenFromGlm(glm::mat4 const& m);
    glm::vec3 eigenToGlm3(Eigen::Vector3f const& v);
    Eigen::Vector3f glmToEigen(glm::vec3 const& v);
    glm::vec3 glmFromEigen3(Eigen::Vector3f const& v);
    Eigen::Vector3f eigenFromGlm(glm::vec3 const& v);
