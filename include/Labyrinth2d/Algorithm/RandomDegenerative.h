...
template <class URNG>
void Labyrinth2d::Algorithm::RandomDegenerative::operator()(URNG& g, SubGrid const& subGrid,
                                                            ...
                                                            std::chrono::milliseconds const* timeout)
{
    ...
    while (destructedWalls < destructedWallTotal && !walls.empty())
    {
        ...
        if (cycleOperations && cyclePause.count() && !(destructedWalls % cycleOperations))
            sleep(cyclePause);
