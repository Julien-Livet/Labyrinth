...
*/
    ...
    scrollArea.show();
    scrollArea.move(QRect{QPoint{}, QGuiApplication::screenAt(scrollArea.pos())->size()}.center()
                    - QRect{QPoint{}, scrollArea.size()}.center());
    scrollArea.setFocusPolicy(Qt::WheelFocus);

    auto& qpr{wr->qPainterRenderer};
    ...
    auto const player3Id{l.addPlayer(l.grid().rows() - 1, l.grid().columns() - 1, {0}, {0}, true)};
    auto const player4Id{l.addPlayer(0, l.grid().columns() - 1, {l.grid().rows() - 1}, {0}, true)};
